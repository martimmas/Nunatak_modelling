...
UserVar.OutputFileName='IceCap-thw0y1n_thk05m';      % Name of the output file correspondent to this run

% Implemented Experiment options
% '-Transient'
UserVar.IOFilesPath = '/mnt/3tb/Ua/runs/SymIceCap';
...
UserVar.TransRestartRef = 'RestartIceCap-spn0y1n_thk05m';

% Choose which member to base initial conditions on
UserVar.SensEnsMember = 'SMP-SpinupDyn';  % Valid for '-Sensitivity-Ensemble' and '-Sensitivity-ModParams' only!
UserVar.ControlMember = '1';     % Valid for '-Control' only!

%% Restart (might be overwritten based on choice of experiment)
CtrlVar.Restart=0;  CtrlVar.WriteRestartFile=1;
CtrlVar.NameOfRestartFiletoWrite=[UserVar.IOFilesPath,'/ResultsFiles/Restart',UserVar.OutputFileName,'.mat'];
...
switch CtrlVar.Experiment
    ...
    case 'Transient'  
        ...
        if contains(UserVar.TransRestartRef,'/')
            ...
            CtrlVar.NameOfRestartFiletoRead=[UserVar.IOFilesPath,'/ResultsFiles/',UserVar.TransRestartRef];
        end
        ...
        UserVar.MassBalance.Evolution    = 'scaled'; % 'step'|'scaled'|'none/default' (linear is the default)
        UserVar.MassBalance.ScalingFile  = [UserVar.IOFilesPath ,'/InputFiles/LR04_scaled_curve.mat']; % THIS WILL ONLY SCALE START AND END VALUES
        ...
        UserVar.MassBalance.EndValue     = -0.75;    % Set mass balance value for end of run (m/yr at upstream boundary) [default: -0.1 m/a]        
        ...
        UserVar.NunatakShape = '0y'; % {0y|0x|Tu}
        UserVar.NunatakSpacing = 5e3; % how far away the nunataks are from each other, in m            
...
CtrlVar.ThickMin=0.5; % threshold [m] for the minimum thickness
...
CtrlVar.TriNodes=3 ;
CtrlVar.MeshSize=8e3;
CtrlVar.MeshSizeMin=2.5e2;
...
CtrlVar.AdaptMeshRunStepInterval=100; %6 ; % Number of run-steps between mesh adaptation
